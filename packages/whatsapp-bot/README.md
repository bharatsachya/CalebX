...
monorepo — see the matchmaking project brief. CALEBX's own WhatsApp presence
used to live in this package; it has been repurposed, and CALEBX continues on
Telegram only (`packages/telegram-bot`).

A thin boundary: it receives webhooks, verifies them, and translates between
WhatsApp's wire format and `@calebx/matchmaking`'s Postgres-backed domain.
