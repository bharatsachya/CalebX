...
function isFileSizeApplicable(path: string): boolean {
  ...
  // CLAUDE.md scopes the 300-line rule to packages/* and deployables.
  // scripts/ holds dev-tooling and one-off drivers — out of scope.
...
export function checkReadmeMd(files: string[]): void {
  ...
  const missing = [...required].filter(
    (ctx) => !existsSync(ctx) && !isInIndex(ctx),
  );
  if (missing.length === 0) {
    return;
  }
